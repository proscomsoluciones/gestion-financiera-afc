...
<head>
    <meta charset="UTF-8">
    <title>Libro de Respaldos - {{ $periodTitle }}</title>
    <style>
        @page {
            margin: 30px 40px;
        }
...
<body>
        ...
        <tr>
            ...
            <td style="vertical-align: middle; padding-left: 8px;">
                ...
                <p style="font-size: 11px; color: #0f172a; margin: 3px 0 0 0; font-weight: 900; text-transform: uppercase;">
                    LIBRO DE RESPALDOS DE EGRESOS & COMPROBANTES
                </p>
            </td>
        ...
        <table style="width: 100%; border-collapse: separate; border-spacing: 5px 0;">
            <tr>
                ...
                <td style="width: 33.3%; background-color: #eff6ff; border: 1.5px solid #3b82f6; border-radius: 6px; padding: 8px; text-align: center;">
                    ...
                    <span style="font-size: 12px; font-weight: 900; color: #1e40af; display: block; margin-top: 2px;">${{ number_format($balance, 0, ',', '.') }} CLP</span>
                </td>
            </tr>
        </table>
    </div>
    ...
    <div class="section-title">
        DETALLE DE EGRESOS REGISTRADOS EN EL PERÍODO
    </div>
            ...
            @empty
                <tr>
                    <td colspan="4" style="text-align: center; padding: 20px; color: #888;">
                        No se registraron egresos en este período.
                    </td>
                </tr>
            @endforelse
        </tbody>
    ...
    <!-- PAGE 2+: ANEXO DE COMPROBANTES -->
    @if($expenses->count() > 0)
        <div class="page-break"></div>

        <div class="appendix-header">
            <h2>ANEXO DE COMPROBANTES</h2>
            <p>{{ $periodTitle }}</p>
        </div>
        ...
        @foreach($expenses as $exp)
            <div class="receipt-card">
                <div class="receipt-title">
                    {{ \Carbon\Carbon::parse($exp->date)->format('d/m/Y') }} - {{ $exp->concept }} - ${{ number_format($exp->amount, 0, ',', '.') }}
                </div>
                <div class="receipt-image-container">
                    @if(!empty($exp->receipt_base64))
                        <img src="{{ $exp->receipt_base64 }}" alt="Comprobante {{ $exp->folio_number }}">
                    @else
